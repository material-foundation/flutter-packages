...
- Text theme support. Every font family now _also_ has a `TextTheme` method. For example, the `Lato` font now has `GoogleFonts.latoTextStyle()` and `GoogleFonts.latoTextTheme()`. See README for more examples.
...
- Initial release: supports all 960 fonts and variants from fonts.google.com. - ttf files are downloaded via http on demand, and saved to local disk so that they can be loaded
